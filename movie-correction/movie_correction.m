...
%name = '120s.tif'; % file path and name in .tif format

[filename,path] = uigetfile('*.tif','Find the tif movie file for the correction?')
...
[save_name,path] = uiputfile('*.tif','Where to save the corrected movie')
filename = fullfile(path,save_name);
res = saveastiff(M2, filename) % save the corrected movie as tif file
